...
import { getGroqAnalysis, abortGroqAnalysis, GROQ_MODELS } from '../utils/groq';

// ── Groq brand colour ─────────────────────────────────────────────────────────
const GROQ_COLOR  = '#f55036';
const GROQ_COLOR2 = '#ff8c69';
...
function parseRetrySeconds(msg = '') {
  const m = msg.match(/retry.*?(\d+(?:\.\d+)?)\s*s/i) || msg.match(/(\d+(?:\.\d+)?)\s*s/i);
...
// ── Groq logo ─────────────────────────────────────────────────────────────────
function GroqLogo({ size = 16 }) {
  return (
    <svg width={size} height={size * 0.8} viewBox="0 0 40 32" fill="none">
      <circle cx="20" cy="16" r="14" stroke={GROQ_COLOR} strokeWidth="3.5" fill="none"/>
      <circle cx="20" cy="16" r="6"  fill={GROQ_COLOR}/>
    </svg>
  );
}
        ...
        <>
          ...
          <div style={{ display: 'flex', gap: 6, marginBottom: 10 }}>
            <div style={{ flex: 1, background: C.bg, borderRadius: 4, padding: '6px 8px', border: `1px solid ${C.border}` }}>
              ...
              <div className="mono" style={{ fontSize: 11, fontWeight: 700, color: GROQ_COLOR }}>{trade.risk_reward || '—'}</div>
            </div>
            <div style={{ flex: 1, background: C.bg, borderRadius: 4, padding: '6px 8px', border: `1px solid ${C.border}` }}>
          ...
          <div style={{ marginBottom: 10 }}>
            {result.key_reasons.map((r, i) => (
              <div key={i} style={{
                ...
                borderLeft: `2px solid ${i % 2 === 0 ? GROQ_COLOR : GROQ_COLOR2}60`,
...
// ── Main component ────────────────────────────────────────────────────────────
export default function GroqPanel({ symbol, timeframe, ticker, inds, signal, candles, market = 'spot' }) {
  const { colors: C } = useTheme();
  ...
  const runAnalysis = useCallback(async (data, mdl) => {
    setLoading(true);
    setError(null);
    setRetryIn(null);
    try {
      const res = await getGroqAnalysis(data, mdl);
      setResult(res);
      ...
      if (err.message === 'ABORTED') {
        ...
        pendingData.current = { data, model: mdl };
        setRetryIn(secs);
        setError('rate-limit');
      } else {
        pendingData.current = null;
  ...
  return (
    <div style={{
      background: C.card,
      border: `1px solid ${GROQ_COLOR}30`,
          ...
          <div>
            <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
              ...
              <span style={{
                ...
                color: GROQ_COLOR, background: `${GROQ_COLOR}18`,
                border: `1px solid ${GROQ_COLOR}40`, padding: '1px 6px', borderRadius: 3,
              }}>
                Free · Fast
              </span>
              {collapsed && sigBadge && (
            ...
            {!collapsed && (
              <div style={{ fontFamily: "'Raleway', sans-serif", fontSize: 9, color: C.muted }}>
                Groq · Llama / Mixtral · AI trade signal · Entry · TP · Stop Loss
              </div>
            )}
          </div>
        </div>
        ...
        <>
            ...
            {loading ? (
              ...
              <button
                ...
                style={{
                  ...
                  border: `1px solid ${GROQ_COLOR}60`,
                  background: `${GROQ_COLOR}15`,
                  color: candles?.length ? GROQ_COLOR : C.muted,
          ...
          {loading && (
            <div style={{
              ...
              border: `1px solid ${GROQ_COLOR}30`, borderRadius: 6, marginBottom: 10,
            }}>
              <div style={{ display: 'flex', gap: 3 }}>
                {[GROQ_COLOR, GROQ_COLOR2, GROQ_COLOR, GROQ_COLOR2].map((col, i) => (
                  <div key={i} style={{
              ...
              <div style={{ fontFamily: "'Raleway', sans-serif", fontSize: 11, color: GROQ_COLOR }}>
          ...
          {error && !loading && (
            error === 'rate-limit' ? (
              <div style={{
                fontFamily: "'Raleway', sans-serif", fontSize: 11,
                color: GROQ_COLOR2, background: `${GROQ_COLOR}12`,
                border: `1px solid ${GROQ_COLOR}40`, borderRadius: 5,
                padding: '10px 12px', marginBottom: 10, textAlign: 'center',
              }}>
                <div style={{ fontSize: 13, fontWeight: 700, marginBottom: 4 }}>⏳ Rate limit hit</div>
                <div style={{ color: '#b8cce0', marginBottom: 6 }}>
                  Auto-retrying in <span style={{ color: GROQ_COLOR2, fontWeight: 700, fontSize: 14 }}>{retryIn}s</span>
                </div>
                <button onClick={abort} style={{
                  marginTop: 4, fontFamily: "'Raleway', sans-serif", fontSize: 10,
          ...
          {!result && !loading && !error && (
            <div style={{
              ...
              Click "Analyse with Groq" to get a free AI trade signal
            </div>
          )}
        </>
      )}
