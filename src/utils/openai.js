/**
 * OpenAI integration for CryptoEdge Pro
 * Routes through the local proxy server (server.js on :3001)
 * which forwards to api.openai.com using OPENAI_API_KEY.
 *
 * Start the proxy first:  node server.js
 * Then start the app:     npm start
 */

const PROXY_URL  = 'http://localhost:3001/api/openai';
export const OPENAI_MODELS = [
  { id: 'gpt-4o-mini', label: 'GPT-4o mini'  },
  { id: 'gpt-4o',      label: 'GPT-4o'       },
];

// ── Prompt builder (identical analysis context to claude.js) ──────────────────
...
Key instructions:
- Use the INDICATOR TRENDS series to assess momentum direction, not just the snapshot value.
- Scale your entry/exit prices and time horizon to the ${timeframe.label} timeframe.
- Use the full ${candleCount}-candle price action table to identify support/resistance levels.
...
export async function abortOpenAIAnalysis() {
  if (_abortController) { _abortController.abort(); _abortController = null; }
  try {
    await fetch('http://localhost:3001/api/openai/abort', { method: 'POST' });
  } catch {}
}

// ── Main API call ─────────────────────────────────────────────────────────────
export async function getOpenAIAnalysis(data, model = 'gpt-4o-mini') {
