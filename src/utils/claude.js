/**
 * Claude AI integration for CryptoEdge Pro
...
// Trailing series of indicator values (oldest → newest), for trend direction
...
function buildPrompt({ symbol, timeframe, ticker, inds, signal, candles, market }) {
  ...
  // ── Indicator series (last 6 readings, oldest → newest) ────────────────────
  ...
  // ── Candle table (scaled to timeframe) ─────────────────────────────────────
...
${market === 'futures' ? 'This is a leveraged perpetual futures position — factor in funding rates, liquidation risk, and appropriate position sizing for leveraged exposure. Stop losses are critical.' : 'This is a spot position — focus on accumulation strategy, hodl vs. trade framing, and capital preservation.'}

Key instructions:
- Use the INDICATOR TRENDS series to assess momentum direction, not just the snapshot value. A rising RSI series matters more than its absolute level.
- Scale your entry/exit prices and time horizon to the ${timeframe.label} timeframe — a scalp on 5m looks very different from a swing on 4h.
- Use the full ${candleCount}-candle price action table to identify support/resistance levels and candlestick context.
...
Respond with ONLY valid JSON (no markdown fences, no text outside JSON):
{
  ...
  "trade": {
    ...
    "entry": <number — ideal entry price>,
    "entry_note": "<brief entry tactic, e.g. 'limit at support retest'>",
    "tp1": <number — first take profit>,
    "tp2": <number — second take profit>,
    "stop_loss": <number — hard stop>,
...
const SYSTEM_PROMPT = 'You are an expert cryptocurrency technical analyst. You always respond with ONLY valid JSON — no markdown code fences, no preamble, no explanation outside the JSON object. Your signal must be one of: LONG, SHORT, or HOLD. LONG means buy/go long. SHORT means sell/go short (or exit longs). HOLD means no trade.';

// ── Abort in-flight request ───────────────────────────────────────────────────
export async function abortClaudeAnalysis() {
  try {
    await fetch('http://localhost:3001/api/claude/abort', { method: 'POST' });
  } catch {}
}
